...
// Specific Stripe links for different tiers
const STRIPE_LINKS: Record<string, string> = {
  'Starter': 'https://buy.stripe.com/test_4gMaEX8wP1rf99Z3aqfMA02',      // 50,000 tokens
  'Professional': 'https://buy.stripe.com/test_fZufZh14nb1P2LBh1gfMA03', // 200,000 tokens
  'Enterprise': 'https://buy.stripe.com/test_28E28rfZh4Dr1Hx5iyfMA04',   // 1,000,000 tokens
  'DEFAULT': 'https://buy.stripe.com/test_3cI4gz9ATee12LBeT8fMA01'       // All other upgrades
};
...
const SettingsModal: React.FC<SettingsModalProps> = ({ user, onUpdateUser, onLogout, onClose, initialTab = 'profile' }) => {
  ...
  const [enterpriseTokens, setEnterpriseTokens] = useState(
    user.tier === 'Enterprise' ? user.tokenLimit : 1000000
  );

  const calculateEnterprisePrice = (tokens: number) => {
    return `$${(tokens / 1000000) * 50}.00`;
  };

  /**
   * Processes the saving of basic user configuration settings.
   */
  const handleApply = () => {
  ...
  const handleStripeRedirect = () => {
    ...
    // Select appropriate link based on tier
    const link = STRIPE_LINKS[pendingPlan.tier] || STRIPE_LINKS['DEFAULT'];
    
    // Open the selected Stripe link
    window.open(link, '_blank');
    
    // Move to processing state after redirection
    setTimeout(() => {
  ...
  /**
   * Verified the payment against a simulated backend 'hook'.
   * Successful payment is the ONLY guarantee for an upgrade.
   */
  const verifyPaymentAndFinalize = () => {
    ...
    // Simulate checking the "Stripe Hook" - only 70% chance of success on any given click 
    // to simulate the time delay for webhooks to propagate.
    ...
    setTimeout(() => {
      if (isVerified) {
        ...
        // Verification failed - Stripe has not yet signaled "success" to our hook
  ...
  const handleDownloadInvoice = (invoice: Invoice) => {
    alert(`Generating high-resolution PDF for Invoice ${invoice.id}...`);
  };
          ...
          <div className="absolute inset-0 z-[200] flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-md animate-in fade-in duration-300">
            ...
            {(purchaseState === 'processing' || purchaseState === 'verifying') && (
              <div className="max-w-md w-full bg-white dark:bg-slate-900 rounded-[2.5rem] overflow-hidden shadow-2xl p-10 text-center space-y-6">
                <div className="relative">
                  <i className={`fa-solid ${purchaseState === 'verifying' ? 'fa-circle-notch fa-spin text-emerald-500' : 'fa-hourglass-half animate-pulse text-amber-500'} text-4xl`}></i>
                </div>
                <h4 className="text-xl font-black uppercase tracking-tight">
                  {purchaseState === 'verifying' ? 'Verifying Hook...' : 'Awaiting Stripe Hook'}
                </h4>
                <p className="text-xs text-slate-500 leading-relaxed px-4">
                  {purchaseState === 'verifying' 
                    ? 'Synchronizing with Stripe secure transaction logs...' 
                    : "Once you've completed your purchase in the Stripe window, click the button below to confirm the transaction hook."}
                </p>
                <button 
                  onClick={verifyPaymentAndFinalize} 
                  disabled={purchaseState === 'verifying'}
                  className="w-full py-4 bg-slate-900 dark:bg-emerald-700 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl flex items-center justify-center gap-2 hover:bg-emerald-800 transition-all disabled:opacity-50"
                >
            ...
            {purchaseState === 'failed' && (
              <div className="max-w-md w-full bg-white dark:bg-slate-900 rounded-[2.5rem] overflow-hidden shadow-2xl p-10 text-center space-y-6 animate-in shake duration-500">
                <div className="w-20 h-20 bg-rose-100 dark:bg-rose-900/30 rounded-3xl flex items-center justify-center text-rose-600 mx-auto">
            ...
            {purchaseState === 'success' && (
              <div className="text-center animate-in zoom-in duration-500">
                <div className="w-24 h-24 bg-emerald-500 rounded-full flex items-center justify-center mx-auto mb-6 shadow-2xl border-4 border-emerald-400/50">
        ...
        <header className="px-8 py-6 bg-emerald-800 dark:bg-emerald-950 text-white shrink-0 flex justify-between items-center border-b border-white/10">
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center backdrop-blur-sm border border-white/20">
              <i className="fa-solid fa-gear text-emerald-300"></i>
            </div>
            <div>
          ...
          {tabs.map(tab => (
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-4 text-[10px] font-black uppercase tracking-widest transition-all relative ${
                activeTab === tab.id ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'
              }`}
            >
          ...
          {activeTab === 'profile' && (
            <div className="space-y-8 animate-in fade-in slide-in-from-top-2">
              <div className="flex items-center gap-6 p-6 bg-slate-50 dark:bg-slate-800 rounded-[2.5rem] border border-slate-100 dark:border-slate-700">
                ...
                <button 
                  onClick={() => alert('Feature to update avatar arriving in next patch.')}
                  className="px-4 py-2 bg-slate-200 dark:bg-slate-700 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-emerald-600 hover:text-white transition-all"
                >
                  Edit Profile Photo
                </button>
              </div>
          ...
          {activeTab === 'billing' && (
            <div className="space-y-8 animate-in fade-in slide-in-from-top-2">
              <div className="bg-emerald-600 p-8 rounded-[2.5rem] text-white flex justify-between items-center shadow-xl">
                <div>
                  <h5 className="text-[10px] font-black text-emerald-200 uppercase tracking-widest mb-1">Total Pack Limit</h5>
                  <p className="text-4xl font-black">{formData.tokenLimit.toLocaleString()} Tokens</p>
                </div>
                <div className="text-right">
                  <p className="text-[10px] font-black text-emerald-200 uppercase mb-1">Tier</p>
                  <p className="text-lg font-black bg-white/20 px-4 py-1 rounded-full">{formData.tier}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {PLANS.map(plan => {
                  ...
                  return (
                    <div key={plan.tier} className={`p-6 rounded-[2rem] border-2 transition-all flex flex-col ${isActive ? 'bg-emerald-50 dark:bg-emerald-950/20 border-emerald-500' : 'bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700 hover:border-slate-200'}`}>
                      <div className="flex justify-between items-center mb-4">
                        <span className="text-[10px] font-black uppercase text-slate-400">{plan.tier}</span>
                      ...
                      {plan.tier === 'Enterprise' && (
                        <div className="mb-4">
                          ...
                          <select 
                            ...
                            className="w-full bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2 text-xs font-bold focus:ring-1 focus:ring-emerald-500 outline-none text-slate-900 dark:text-white"
                          >
                            {ENTERPRISE_OPTIONS.map(opt => (
                              <option key={opt} value={opt}>{opt.toLocaleString()} Tokens - {calculateEnterprisePrice(opt)}</option>
                            ))}
                          </select>
                        </div>
                      )}
                      ...
                      <div className="flex-1 space-y-2 mb-6">
                        {plan.features.map((f, idx) => (
                          <p key={idx} className="text-[10px] text-slate-500 dark:text-slate-400 flex items-start gap-2">
                            <i className="fa-solid fa-check text-emerald-500 mt-0.5"></i> 
                            <span className="leading-tight">{f}</span>
                          </p>
                        ))}
                      </div>
                      <button onClick={() => initiatePlanSelection(plan.tier)} className={`w-full py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${isActive ? 'bg-emerald-600 text-white shadow-lg' : 'bg-slate-900 text-white hover:bg-emerald-700 active:scale-95'}`}>
                        {isActive ? 'Current Active Plan' : `Refill ${price}`}
                      </button>
                    </div>
                  );
                })}
                  ...
                  {formData.invoices && formData.invoices.length > 0 ? (
                    <table className="w-full text-left">
                      <thead className="bg-slate-100 dark:bg-slate-900">
                        <tr>
                          <th className="px-6 py-3 text-[9px] font-black uppercase text-slate-500">Invoice ID</th>
                          <th className="px-6 py-3 text-[9px] font-black uppercase text-slate-500">Amount</th>
                          <th className="px-6 py-3 text-right pr-6 text-[9px] font-black uppercase text-slate-500">Action</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                        {formData.invoices.map(inv => (
                          <tr key={inv.id} className="hover:bg-white dark:hover:bg-slate-700 transition-all">
                            <td className="px-6 py-4">
                              <p className="text-[11px] font-black">{inv.id}</p>
                              <p className="text-[9px] text-slate-400">{inv.date}</p>
                            </td>
                            <td className="px-6 py-4 text-[11px] font-bold text-emerald-600">{inv.amount}</td>
                            <td className="px-6 py-4 text-right pr-6">
                              <button onClick={() => handleDownloadInvoice(inv)} className="text-slate-400 hover:text-emerald-600 transition-colors"><i className="fa-solid fa-download"></i></button>
                            </td>
                          </tr>
                        ))}
                      </tbody>
            ...
            <div className="space-y-10 animate-in fade-in slide-in-from-top-2">
              <section className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button onClick={() => setFormData(p => ({ ...p, preferences: { ...p.preferences, theme: 'light' } }))} className={`p-6 rounded-[2rem] border-2 transition-all flex flex-col items-center gap-3 ${formData.preferences.theme === 'light' ? 'bg-white border-emerald-500 shadow-xl' : 'bg-slate-50 dark:bg-slate-800 border-transparent text-slate-400'}`}>
                  ...
                  <button onClick={() => setFormData(p => ({ ...p, preferences: { ...p.preferences, theme: 'dark' } }))} className={`p-6 rounded-[2rem] border-2 transition-all flex flex-col items-center gap-3 ${formData.preferences.theme === 'dark' ? 'bg-slate-800 border-emerald-500 shadow-xl text-white' : 'bg-slate-50 dark:bg-slate-800 border-transparent text-slate-400'}`}>
                    <i className="fa-solid fa-moon text-2xl text-blue-400"></i>
              ...
              <section className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {LANGUAGES.map(lang => (
                    <button 
                      key={lang.code}
                      onClick={() => setFormData(p => ({ ...p, preferences: { ...p.preferences, language: lang.code as Language } }))}
                      className={`py-4 rounded-2xl border-2 transition-all flex flex-col items-center gap-2 ${formData.preferences.language === lang.code ? 'bg-emerald-50 dark:bg-emerald-950 border-emerald-500 shadow-lg' : 'bg-slate-50 dark:bg-slate-800 border-transparent text-slate-400'}`}
                    >
              ...
              <section className="bg-slate-50 dark:bg-slate-800 p-6 rounded-[2rem] flex items-center justify-between">
                <div>
                  <h5 className="text-sm font-bold text-slate-900 dark:text-white">Auto-Save Cloud Drafts</h5>
                  <p className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-black tracking-widest mt-1">Saves audit progress every 5 minutes</p>
                </div>
                <button 
                  onClick={() => setFormData(p => ({ ...p, preferences: { ...p.preferences, autoSave: !p.preferences.autoSave } }))}
                  className={`w-14 h-7 rounded-full relative transition-all duration-300 ${formData.preferences.autoSave ? 'bg-emerald-600' : 'bg-slate-300 dark:bg-slate-700'}`}
                >
                  <div className={`w-5 h-5 bg-white rounded-full absolute top-1 transition-all ${formData.preferences.autoSave ? 'left-8' : 'left-1'}`}></div>
                </button>
              </section>
            </div>
          )}
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-top-2">
              <div className="bg-slate-50 dark:bg-slate-800 p-8 rounded-[2.5rem] space-y-8">
                <div className="flex items-center justify-between">
                  <div>
                    <h5 className="text-sm font-bold text-slate-900 dark:text-white">Compliance Alerts</h5>
                    <p className="text-[10px] text-slate-500 uppercase tracking-widest font-black mt-1">New RSPO Indicator & P&C Updates</p>
                  </div>
                  <button onClick={() => toggleNotification('compliance')} className={`w-14 h-7 rounded-full relative transition-all duration-300 ${formData.notifications.compliance ? 'bg-emerald-600' : 'bg-slate-300 dark:bg-slate-700'}`}>
                    <div className={`w-5 h-5 bg-white rounded-full absolute top-1 transition-all ${formData.notifications.compliance ? 'left-8' : 'left-1'}`}></div>
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h5 className="text-sm font-bold text-slate-900 dark:text-white">Billing & Usage Notices</h5>
                    <p className="text-[10px] text-slate-500 uppercase tracking-widest font-black mt-1">Token Refills, Invoices & Balance Alerts</p>
                  </div>
                  <button onClick={() => toggleNotification('billing')} className={`w-14 h-7 rounded-full relative transition-all duration-300 ${formData.notifications.billing ? 'bg-emerald-600' : 'bg-slate-300 dark:bg-slate-700'}`}>
                    <div className={`w-5 h-5 bg-white rounded-full absolute top-1 transition-all ${formData.notifications.billing ? 'left-8' : 'left-1'}`}></div>
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h5 className="text-sm font-bold text-slate-900 dark:text-white">System Service Updates</h5>
                    <p className="text-[10px] text-slate-500 uppercase tracking-widest font-black mt-1">Maintenance, Feature Drops & Security Logs</p>
                  </div>
                  <button onClick={() => toggleNotification('system')} className={`w-14 h-7 rounded-full relative transition-all duration-300 ${formData.notifications.system ? 'bg-emerald-600' : 'bg-slate-300 dark:bg-slate-700'}`}>
                    <div className={`w-5 h-5 bg-white rounded-full absolute top-1 transition-all ${formData.notifications.system ? 'left-8' : 'left-1'}`}></div>
                  </button>
                </div>
              </div>

              <div className="p-6 bg-amber-50 dark:bg-amber-950/20 rounded-[2rem] border border-amber-100 dark:border-amber-900 flex gap-4">
                <i className="fa-solid fa-circle-info text-amber-600 text-lg mt-1"></i>
                <p className="text-[11px] text-amber-800 dark:text-amber-200 leading-relaxed font-medium">
                  Push notifications require browser permission. You will be prompted for access next time a critical update is pushed to your workspace.
                </p>
              </div>
            </div>
          )}
